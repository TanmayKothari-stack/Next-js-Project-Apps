...
router.get('/', async (req, res) => {
    ...
    const result = await pool.query("select * from chats where (sender_id = $1 and receiver_id = $2 or sender_id = $2 and receiver_id = $1) and (deleted is null or deleted <> $1) order by id asc",
...
router.put("/update", async (req, res) => {
    ...
    await pool.query("update chats set replyed_message = $1 where replyed_message_id = $2 returning * ",
        [message, id]
    );
...
router.delete('/delete', async (req, res) => {
    ...
    for (const message of messages) {
        ...
        [senderId] = hashids.decode(message.sender_id);
        ...
        reply_update as(
                update chats set replyed_message_deleted = $1
                where replyed_message_id = $2
            )
...
router.post("/reply", async (req, res) => {
    let { sender_id, sender_name, receiver_id, receiver_name, message, replyMessageId, replyMessage, replyMessageOffset } = req.body;
    ...
    const result = await pool.query("insert into chats (sender_id, sender_name, receiver_id, receiver_name, message, replyed_message_id, replyed_message, replyed_message_offset, time) values($1, $2, $3, $4, $5, $6, $7, $8, $9) returning * ",
        [sender_id, sender_name, receiver_id, receiver_name, message, replyMessageId, replyMessage, replyMessageOffset, "now()"]
    );
